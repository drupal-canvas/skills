...
| ------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **Drupal menu**     | `useJsonApiClient()` → `client.getResource('menu_items', menuMachineName)` + `sortMenu(data)` | Editors maintain links under Structure → Menus; same nav reused across pages                   |
| **Page breadcrumb** | `usePageContext()` → `breadcrumbs` (iterable of `key`, `text`, `url`)                         | Trail reflects current route / hierarchy; not a menu resource                                  |
...
- Read the client with `useJsonApiClient()` from `drupal-canvas/react`.
- SWR key: `client && menuName ? ['menu_items', menuName] : null`.
- Links for rendering: `Array.from(sortMenu(data))` when data is available,
  including SWR fallback data; otherwise **`FALLBACK_LINKS`** (always define
  this constant). Do not mask expired draft sessions with static links.
...
- Use **`usePageContext()`** from `drupal-canvas/react`, not `menu_items`. Call
  it unconditionally, then handle null context or empty breadcrumbs.
- Workbench's empty breadcrumbs are intentional; verify real trails on a routed
  page. Preserve Drupal-generated URLs unless application routing adapts them.
